...
const ErrorEnPago = () => {
  ...
  const [newOrderRetry, setNewOrderRetry] = useState({})
  const formTransbank = useRef()
  ...
  // Refrescar la orden para optener nuevo token para reintentar
  useEffect(() => {
    if (Object.keys(order).length === 0 || !order.token_ws) {
      return
    }
    clienteAxios.get(`pre-order/transaction/${order.token_ws}/retry`).then(
      (response) => {
        setNewOrderRetry(response.data)
      }
    )
  }, [order])
  ...
  const hangleRetry = () => {
    ...
    try {
      if (!newOrderRetry.form_action || !newOrderRetry.token_ws) {
        console.log('Error al momento de refrescar newOrderRetry')
        return
      }
      formTransbank.current.submit()
    } catch (error) {
      console.log(error)
    }
  }
                ...
                <div className="max-w-lg mx-auto">
                  <div className="w-full rounded-lg bg-white p-5">
                    ...
                    <p className="text-2xl tracking-wide">
                      Hemos tenido problemas al procesar el pago.
                    </p>
                    ...
                    disabled={loadingRetry ? true : false}
                    onClick={hangleRetry}>
                      Reintentar Pago
                      {loadingRetry && (
                        <svg
                  ...
                  {
                    (newOrderRetry.form_action && newOrderRetry.token_ws) && (
                      <form ref={formTransbank} className="hidden" method="post" action={ newOrderRetry.form_action } >
                        <input type="hidden" name="token_ws" value={ newOrderRetry.token_ws } />
                        <input type="submit" value="Reintentar Pago" />
                      </form>
                    )
                  }
                </div>
